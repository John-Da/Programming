...
class Buttons:
    def __init__(self, master, bgColor, fFamily, redColor, resumeImg):
        ...
        self.addFrm = customtkinter.CTkFrame(
            master=self.master,
            width=720,
            height=520,
            corner_radius=15,
        ...
        self.scoreLabel = customtkinter.CTkLabel(
            master=self.addFrm,
            text=f"Score: {self.totalScore}",
            font=(fFamily, 28, "bold"),
            text_color=redColor,
        )
        self.scoreLabel.place(relx=0.15, rely=0.12, anchor=tkinter.CENTER)

        self.chancesLabel = customtkinter.CTkLabel(
            master=self.addFrm,
        ...
        resumeBtnImg = ImageTk.PhotoImage(
            Image.open(resumeImg).resize((40, 40), Image.AFFINE)
        )
        self.resumeBtn = customtkinter.CTkButton(
            master=self.addFrm,
            image=resumeBtnImg,
            text="",
    ...
    def randomBtn(self):
        ...
        for idx, value in enumerate(self.randomPlace):
            btn = customtkinter.CTkButton(
                master=self.addFrm,
                text=str(value),
                text_color=self.bgColor,
                width=100,
                height=50,
                font=(self.fFamily, 16, "bold"),
            )
            btn.place(
                relx=0.25 + (idx % 2) * 0.5,
                rely=0.5 + (idx // 2) * 0.2,
    ...
    def end_game(self):
        self.clear_buttons()
        endLabel = customtkinter.CTkLabel(
            master=self.addFrm,
            text=f"Game Over! Final Score: {self.totalScore}",
            font=(self.fFamily, 28, "bold"),
            text_color=self.redColor,
        )
